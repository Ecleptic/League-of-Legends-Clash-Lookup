...
export class Clash {
    ...
    async run(summonerName, key) {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            // Teammates
            await this.initPlayerData(this.teammates, teammates);
    ...
    async initPlayerData(savedTeammates, curTeammates) {
        for (let player of curTeammates) {
            if (savedTeammates[player.name] == undefined) {
                savedTeammates[player.name] = {};
                savedTeammates[player.name].wins = 0;
                savedTeammates[player.name].losses = 0;

                // Add icon url
                savedTeammates[player.name].iconUrl = await gameInfo.getPlayerIconUrl(player.iconId);
            }
        }
    }
