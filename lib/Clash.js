...
export class Clash {
    ...
    async run(summonerName, key) {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            await this.initWinrateData(this.champWinrates, champName, "champion");

            // Enemy bans
            for (let ban of enemyBanList) {
                await this.initWinrateData(this.enemyBans, ban, "champion");
            }

            // Ally Bans
            for (let ban of allyBanList) {
                await this.initWinrateData(this.allyBans, ban, "champion");
            }

            // Teammates
            for (let teammate of teammates) {
                await this.initWinrateData(this.teammates, teammate, "player");
            }
            ...
            if (i == matches.length - 1) {
                ...
                let teammateArr = this.sortWinrates(this.teammates);
    ...
    async initWinrateData(winrateObj, varName, type) {
    ...
    async addResult(result, champName, role, allyBanList, enemyBanList, teammates) {
        ...
        // Teammates
        for (let teammate of teammates) {
            this.teammates[teammate][resultKey]++;
        }
    }
