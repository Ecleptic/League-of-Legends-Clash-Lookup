...
export class Match {
    ...
    async getChampionName(summonerName) {
        ...
        return game.getChampionName(champId);
    }
    ...
    async getAllyBans(summonerName) {
        ...
        allyBans.forEach(banData => {
            ...
            output.push(game.getChampionDataFromList(champList, champId).name);
        });
        return output;
    }
    ...
    // Returns an array of teammate {name, iconId}
    async getTeammates(summonerName) {
        ...
        // Get range for teammate names
        let startIndex = (playerIndex < 5) ? 0 : 5;
        ...
        for (let i = startIndex; i < startIndex + 5; i++) {
            ...
            if (i != playerIndex) {
                const data = {
                    name: this.matchData.participantIdentities[i].player.summonerName,
                    iconId: this.matchData.participantIdentities[i].player.profileIcon
                };
                output.push(data)
            }
        }
